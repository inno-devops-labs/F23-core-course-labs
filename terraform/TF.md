...
```bash
[Arseniy172@ terraform]$ terraform state list
docker_container.nginx
docker_image.nginx
[Arseniy172@ terraform]$ terraform state show docker_container.nginx
# docker_container.nginx:
...
```bash
[Arseniy172@ docker-infra]$ terraform output 
...
[Arseniy172@ docker-infra]$ docker ps
...
```bash
[Arseniy172@ cloud-infra]$ terraform state list
...
[Arseniy172@ cloud-infra]$ terraform state show vkcs_compute_instance.compute
# vkcs_compute_instance.compute:
...
### Terraform output
```bash
[Arseniy172@ cloud-infra]$ terraform output 
...
export GITHUB_USER=Arseniy172-test-org
```
Without second it won't work.

**Note:** To accomplish the bonus task I had to use entirely new `Arseniy172-test-org`. That means I was unable to perform `terraform init github_repository.repo core-course-labs` to track my actual course rep by Terraform.

```bash
[Arseniy172@ github]$ terraform state list
...
[Arseniy172@ github]$ terraform state show github_repository.repo
# github_repository.repo:
