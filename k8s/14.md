...
`while true; do curl <python|rust app>; done` in order to generate load.
...
We have lowest load from our Rust application.

The highest load we have from Kubernetes internals (API server, Calico, CoreDNS). This was one-node Kubernetes cluster with small amount of vCPU and RAM (4 vCPU and 4 Gi RAM).
...
`while true; do curl <python|rust app>; done` in order to generate traffic.
