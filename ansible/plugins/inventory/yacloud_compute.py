...
try:
    ...
    from yandex.cloud.resourcemanager.v1.folder_service_pb2_grpc \
        import FolderServiceStub
except ImportError:
    raise AnsibleError(
    ...
    def verify_file(self, path):
        if super(InventoryModule, self).verify_file(path):
            if path.endswith(('yacloud_compute.yml', 'yacloud_compute.yaml')):
                return True
        display.debug(
            "yacloud_compute inventory filename must end "
            "with 'yacloud_compute.yml' or 'yacloud_compute.yaml'")
        return False
